...
Specs_all = np.array(data['data'])
np.random.shuffle(Specs_all)
Specs_test = torch.tensor(Specs_all[0:TestingDataSize, :], device=device_test, dtype=dtype)
...
diff_row = config.get('diff_row', int(TFNum**0.5))

hybnet = HybridNet.ND_HybridNet(diff_row,fnet_path, params_min, params_max, hybnet_size,noise_layer ,device_train)
